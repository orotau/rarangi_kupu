...
def test_get_key():
    assert mw._get_key('rā') == (['r', 'a'], ['r', 'ā'], ['r', 'ā'])
    assert mw._get_key('NGĀ') == (['ng', 'a'], ['ng', 'ā'], ['NG', 'Ā'])
...
def test_sorting():
    ...
    assert sorted(basic_group_shuffled, key=mw._get_key) == ['aroha', 'ēhea', 'hauiti']
    assert sorted(basic_group_shuffled, key=mw._get_key) != sorted(basic_group_shuffled)
    ...
    assert sorted(dash_group_shuffled, key=mw._get_key) == ['takini', 'Taki-o-Autahi', 'Takirā']
    #dash sorts before letters asciibetically
    assert sorted(dash_group_shuffled, key=mw._get_key) != sorted(dash_group_shuffled)
    ...
    assert sorted(space_group_shuffled, key=mw._get_key) == ['tuturuatu', 'tuturu pourewa', 'tuturuwhatu']
    #space sorts before letters asciibetically
    assert sorted(space_group_shuffled, key=mw._get_key) != sorted(space_group_shuffled)

    #order when words differ only by macron
    kaka4 = ['kaka', 'kakā', 'kāka', 'kākā'] #the correct ascending order
    shuffle(kaka4)
    kaka4_shuffled = kaka4
    assert sorted(kaka4_shuffled, key=mw._get_key) == ['kaka', 'kakā', 'kāka', 'kākā']
    ...
    assert sorted(digraph_ng_3_shuffled, key=mw._get_key) == ['tunehe', 'tunu', 'tunga']
    assert sorted(digraph_ng_3_shuffled, key=mw._get_key) != sorted(digraph_ng_3_shuffled)
    ...
    assert sorted(digraph_wh_3_shuffled, key=mw._get_key) == ['kawe', 'kawiti', 'kawhe']
    assert sorted(digraph_wh_3_shuffled, key=mw._get_key) != sorted(digraph_wh_3_shuffled)

    #order when words differ only by case
    assert sorted(['kahurangi','Kahurangi'], key=mw._get_key) == ['Kahurangi', 'kahurangi']
    assert sorted(['Kahurangi','kahurangi'], key=mw._get_key) == ['Kahurangi', 'kahurangi']

    #the one identifiable difference with HPK
    assert sorted(['Tāne', 'tane', 'tāne'], key=mw._get_key) == ['tane', 'Tāne', 'tāne']
