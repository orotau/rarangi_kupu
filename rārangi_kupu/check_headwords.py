...
class HeadwordVariant():

    def __init__(self, letter):
        '''get the headword, variant combos from the spreadsheet'''

        self.letter = letter

        words_dict = SpreadSheet(self.letter).pulldata()
        self.headword_variant = list(words_dict.keys())
    ...
    def validate(self):
        ...
        for k in self.headword_variant:
...
if __name__ == '__main__':
    ...
    if first_argument in pū.all_letters:
        #initialise the HeadwordVariant
        headword_variant = HeadwordVariant(first_argument)
        headword_variant.validate()
        print ('Done - thanks')
    else:
