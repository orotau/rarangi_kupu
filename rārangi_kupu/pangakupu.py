    ...
    for word in nines:
        ...
        for letter in set(word_as_list):
            if len(letter) == 1:
                ...
                print(word, letter, len(children))
    #print(all_children)
...
def get_koru(seed_word=None):
    ...
    nines = [x for x in unique_word_forms if len(x) == 9]

    if seed_word is None:
        seed_word = random.choice([x for x in nines if x not in nines_to_exclude])
...
if __name__ == '__main__':
    ...
    get_all_children_counts_parser = subparsers.add_parser('get_all_children_counts')
    get_all_children_counts_parser.set_defaults(function = get_all_children_counts)
